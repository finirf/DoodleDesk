...
function Desk({ user }) {
  ...
  const canCurrentUserManageMembers = Boolean(
    isCurrentDeskOwner
    || (!selectedDeskMemberRoleLoading && selectedDeskMemberRole === 'manager')
  )
  ...
  const openShelfRenameDialog = React.useCallback((shelfId) => {
    const shelf = deskShelves.find((entry) => entry.id === shelfId)
    if (!shelf) return
  ...
  }, [deskShelves, setShelfActionError, setShelfRenameDialog])

  const closeShelfRenameDialog = React.useCallback(() => {
    setShelfActionError('')
    setShelfRenameDialog({ isOpen: false, shelfId: null, value: '', originalName: '' })
  }, [setShelfActionError, setShelfRenameDialog])

  const submitShelfRenameDialog = React.useCallback((event) => {
    event.preventDefault()
    if (!shelfRenameDialog.shelfId) return

    const didRename = commitDeskShelfRename(shelfRenameDialog.shelfId, shelfRenameDialog.value)
    if (!didRename) return
  ...
  }, [setTutorialDialogOpen, tutorialStorageKey])
  ...
  const openTutorialDialog = React.useCallback(() => {
    setShowMoreMenu(false)
    setShowDeskMenu(false)
    setShowProfileMenu(false)
    setShowNewNoteMenu(false)
    setTutorialDialogOpen(true)
  }, [setShowDeskMenu, setShowMoreMenu, setShowNewNoteMenu, setShowProfileMenu, setTutorialDialogOpen])

  React.useEffect(() => {
    const nextCanvasDimensions = getDeskCanvasDimensions(notes, viewportWidth, sectionHeight, growThreshold)
    setCanvasWidth(nextCanvasDimensions.width)
    setCanvasHeight(nextCanvasDimensions.height)
  ...
  return (
    <DeskCanvasContainer
      ...
      isTouchInteractionMode={isTouchInteractionMode}
    ...
    >
      ...
      >
        ...
        <DeskMoreMenu
          ...
          isCurrentDeskOwner={isCurrentDeskOwner}
          canCurrentUserEditDeskItems={canCurrentUserEditDeskItems}
          showCreatorLabels={currentDesk?.show_item_creator_labels !== false}
          setDeskCreatorLabelsVisible={setDeskCreatorLabelsVisible}
          ...
          onOpenTutorial={openTutorialDialog}
        />
      </DeskTopMenuShell>
      ...
      <DeskCanvasItems
        ...
        editFontWeight={editFontWeight}
        editFontStyle={editFontStyle}
        draggedId={draggedId}
        groupedItemGroupMap={groupedItemGroupMap}
        groupedItemKeys={groupedItemKeys}
        finalizeGroupingSession={finalizeGroupingSession}
        groupItemsByKeys={groupItemsByKeys}
        ...
        setEditFontWeight={setEditFontWeight}
      ...
      <DeskModals
        ...
        deskNameDialog={actionDeskNameDialog}
        submitDeskNameDialog={submitDeskNameDialog}
        deskNameError={actionDeskNameError}
        setDeskNameError={setActionDeskNameError}
        setDeskNameDialog={setActionDeskNameDialog}
        friends={friends}
        getProfileDisplayParts={getProfileDisplayParts}
        toggleInvitedFriend={toggleInvitedFriend}
        closeDeskNameDialog={closeDeskNameDialog}
        deskNameSaving={actionDeskNameSaving}
        ...
        isCurrentDeskOwner={isCurrentDeskOwner}
        canCurrentUserManageMembers={canCurrentUserManageMembers}
        ...
        shelfRenameDialog={shelfRenameDialog}
        setShelfRenameDialog={setShelfRenameDialog}
        submitShelfRenameDialog={submitShelfRenameDialog}
        closeShelfRenameDialog={closeShelfRenameDialog}
        shelfActionError={shelfActionError}
        setShelfActionError={setShelfActionError}
